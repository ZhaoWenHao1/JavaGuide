...
当队列里没有数据时，操作队列会出现异常或返回null，而不是阻塞等待数据。
...
如果BlockingQueue 是空的，从BlockingQueue 取东西的操作将会被阻塞进入等待状态，知道BlockingQueue 添加进了元素才会被唤醒。同样，如果BlockingQueue 是满的，也就是没有空余空间时，试图往队列里存放元素的操作也会被阻塞进入等待状态，知道BlockingQueue 里有剩余空间才会被唤醒继续操作。
...
PriorityBlockingQueue 并发控制采用的是 **ReentrantLock**，队列为**无界队列**（ArrayBlockingQueue 是有界队列，LinkedBlockingQueue 也可以通过在构造函数中传入 capacity 指定队列最大的容量，但是 PriorityBlockingQueue 只能指定初始的队列大小，后面插入元素的时候，**如果空间不够的话会自动扩容**）。
...
使用跳表实现 Map 和使用哈希算法实现 Map 的另外一个不同之处是：哈希并不会保存元素的顺序，而**跳表内所有的元素都是排序的**。因此在对跳表进行遍历时，你会得到一个有序的结果。所以，如果你的应用需要**有序性**，那么跳表就是你不二的选择。JDK 中实现这一数据结构的类是 ConcurrentSkipListMap。
